...
export function MemberCard({ member }: { member: TMember }) {
  const profileData = member.auth.business || member.auth.person;
  return (
    <Link
      href={`/dashboard/user-management/${member.auth.id}`}
      ...
      <div className="flex gap-2">
        ...
        <div>
          <Badge
            variant="default"
            className={`${
              member?.role === "ADMIN" ? "bg-primary" : "bg-[#FF9F41]"
